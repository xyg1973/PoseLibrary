...
def pastPoseRot(posedata,selectobjs,count,progressBar,is_Retarget_Bip_root =False, Retarget_Bip_root = None):
    """
            ...
            for i in range(count):
                for data in posedata:
                    ...
                    if obj in selectobjs:
                        ...
                        if rt.getPropertyController(obj.controller, 'Horizontal') != None:  # 判断是否是质心
                            if is_Retarget_Bip_root == True:
                                obj_Retarget_Bip_root = rt.getNodeByName(Retarget_Bip_root)
                                #判断场景内是否有这个物体和数据中是否有这个物体
                                is_InPosedata = False
                                for dir_temp in posedata:
                                    if dir_temp.get("objname") == obj_Retarget_Bip_root.name:
                                        is_InPosedata =True
                                        obj_Retarget_Bip_root_transform = cmds.data_to_rtMatrix3(dir_temp.get("objtransform"))
                                if obj_Retarget_Bip_root != None and is_InPosedata==True:
                                    offsetmat =  getOffsetMatrix(obj_Retarget_Bip_root_transform,mat3)
                                    newmat3 = offsetmat * obj_Retarget_Bip_root.transform
                            ...
                            if is_Retarget_Bip_root == True and obj.name == Retarget_Bip_root:
                                pass
                            else:
